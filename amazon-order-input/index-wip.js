...
let salesRec, shipName, shipAddressOne,shipAddressTwo, shipCity, shipState, shipZip, itemNumOne, quantityOne, soldFor;
...
function nextOrderBtn() {
    ...
    var filePath = "http://127.0.0.1:5501/master-amazon-order-data.json";//Testing address = http://127.0.0.1:5500/order-data.json
    ...
    .then(function(jsonData) {
        console.log("jsonData orderID 1st");
        console.log(jsonData);
        console.log(jsonData[1].OrderID);
        ...
        function matchOrders(orderNum) {
            return new Promise((resolve, reject) => {
              fetch('http://127.0.0.1:3000/completed-amazon-orders.json')
                .then(response => {
                  if (!response.ok) {
                ...
                .then(data => {
                  ...
                  const findOrderNum = data.find(obj => obj.OrderID === orderNum) || null;
                  console.log("find order num");
                  console.log(findOrderNum);
                 // console.log("order number")
                //  console.log(orderNum);
          
                  resolve(findOrderNum);
                })
                .catch(error => {
        ...
        if(i + 1 <= numOfOrders) {   
            ...
            orderNum = jsonData[i].OrderID; 
           // console.log("first orderNum")
           // console.log(orderNum)
            console.log("order number")
            console.log(orderNum); 
 
            matchOrders(orderNum)
                .then(findOrderNum => {
             // Use findOrderNum inside the callback or any other code
           ...
           shipName = jsonData[i].RecipientName;
           shipAddressOne = jsonData[i].ShipAddress1;
           shipAddressTwo = jsonData[i].ShipAddress2;
           shipCity = jsonData[i].ShipCity;
           shipState = jsonData[i].ShipState;
           shipZip = jsonData[i].ShipZip;
           itemNumOne = jsonData[i].Sku;
           quantityOne = jsonData[i].QuantityPurchased; //This also pulls the first quantity for an order
           soldFor = jsonData[i].ItemPrice;
           ...
           jsonData.forEach(obj => {
               ...
               counts[obj.OrderId].Skus.push(obj.Sku);
               counts[obj.OrderId].QuantityPurchaseds.push(obj.QuantityPurchased);
               counts[obj.OrderId].ItemPrices.push(obj.ItemPrice);
           });
           ...
           if (counts[orderNumber].Skus.length >= 2) {

               //Count the number of customlabel keys the order has
               numOfItems = counts[orderNumber].Skus.length;
               
                   //Iterate through the items
                   for (n = 0; n < numOfItems; n++){
                   //Sets the new variables for multiple items to be displayed   
                   itemCustomLabel = counts[orderNumber].Skus[n];
                   let itemQuantity = counts[orderNumber].QuantityPurchaseds[n];
                   let itemSoldFor = counts[orderNumber].ItemPrices[n];
                   let itemCount = n + 1;
               ...
               function storeOrder() { 
                ...
                //console.log(orderNum)
                sendData(newObjValue);
            }

            function sendData(newObjValue) {
             fetch('http://localhost:3000/sendAmazonObject', {
               method: 'POST',
               headers: {
               ...
               function copyText() {
                    ...
                    //Listens for a click in the customer ID box and changes the value 
                   document.getElementById("order.customer_id").addEventListener("focus", custID);
                   function custID() {
                       document.getElementById("order.customer_id").value = amazon;
                   }
                   //Adds the first item to the clipboard
                   let copyText = orderNum;
                   navigator.clipboard.writeText(copyText);
               
                   //Listens for additional items being pasted
                   ...
                   if(copyText === amazon) {
                           ...
                           //Tests to see if there are multiple items and then iterates through the items
                           if(numOfItems > 1){
                       
                               let items = 0;
               
                               //Checks for multiple items and if so, iterates through them to copy them to the clipboard
                               multiItems();

                               // Define a function called "multiItems" that takes a parameter called "callback"
                                function multiItems(callback) {
                                     // Check if there are more items to process
                                     if (items < numOfItems) {
                                       // Get the label and quantity for the current item
                                        let itemCustomLabel = counts[orderNumber].Skus[items];
                                        let itemQuantity = counts[orderNumber].QuantityPurchaseds[items];
                            ...
                            //Copies a single item to the clipboard
