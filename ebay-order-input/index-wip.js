...
function nextOrderBtn() {
   ...
   .then(function(jsonData) {
       ...
       // Run the processOrders function
    checkOrders(numOfOrders, jsonData);
       
       async function checkOrders(numOfOrders, jsonData) {
       
          if (i + 1 <= numOfOrders) {
            //Looks to see if the order number is in local storage
            let orderNum = jsonData[i].OrderNumber;
            let findOrderNum = await matchOrders(orderNum);
           //let findOrderNum = localStorage.getItem("Order-" + orderNum); 

           console.log("Find Order Num-67");
           console.log(findOrderNum);
           console.log("order num-69");
           ...
           // Check if the current order has two or more custom labels
           if (counts[orderNumber].CustomLabels.length >= 2) {
               ...
               // Get the number of items in the current order
               numOfItems = counts[orderNumber].CustomLabels.length;
               
                   // Iterate through each item in the order
                   for (n = 0; n < numOfItems; n++){
                   // Extract the label, quantity, and sold for value of the current item
                   ...
                   // Generate an item count by adding one to the index (since index is zero-based)
                   let itemCount = n + 1;
                   
                    // Append the current item's information to a HTML element with id "display-items"
                    // Each item's information is displayed in a separate styled div 
                   ...
                   // If the current order does not have two or more items, it is considered a single item order
                    // The single item's information is set to a HTML element with id "display-items"
               ...
               function storeOrder() { 
                ...
                console.log("storeOrder triggered")
                sendData(newObjValue);
            }

            function sendData(newObjValue) {
             fetch('http://localhost:3000/sendEbayObject', {
               method: 'POST',
               headers: {
...
// Make matchOrders return a Promise
function matchOrders(orderNum) {
    return new Promise((resolve, reject) => {
      fetch('http://127.0.0.1:3000/completed-ebay-orders.json')
        .then(response => {
          if (!response.ok) {
        ...
        .then(data => {
          ...
          let order = data.find(obj => obj.OrderNumber === orderNum) || null;
          console.log("find order num in matchOrders");
          console.log(order);
  
          // Resolve the Promise with order
          resolve(order);
        })
        .catch(error => {
          console.error('Error:', error);
